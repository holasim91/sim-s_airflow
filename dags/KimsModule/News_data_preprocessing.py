...
class News_PreProcessing:
    ...
    def content_pre(self, file_name=""):
        ...
        def func(x):
            ...
            tmp3 = re.sub(
                r"[a-zA-Z0-9]+\@[a-zA-Z0-9]+\.[a-z]{1,3}\.[a-z]{1,3}", "", tmp2
            )
            ...
            tmp7 = re.sub(
                r"(동영상 뉴스|클릭|눌러보기|네이버에서|네이버|채널에서|구독해주세요|구독하기|한경닷컴|서울신문|모바일한경|구독신청|뉴스|저작권자|기자|Copyrights|미디어넷|구독하기꿀잼가득|영상보기|네이버 홈에서|한겨레|정기구독|한겨레신문|청춘뉘우스 스냅타임|바로가기)",
                "",
                tmp6,
            )
            tmp8 = re.sub(r"●", "", tmp7)
        ...
        if file_name != "":
            news.to_csv(file_name, encoding="utf-8", index=False)
    ...
    def tokens_ngram(self, file_name=""):
        ...
        for i in range(len(news)):
            try:
                ...
                tokens = [
                    token.lower() for token in tokens if len(token.split("/")[0]) > 1
                ]
                tokens = [
                    token.lower()
                    for token in tokens
                    if token.split("/")[0] not in stop_word
                ]
                ...
                if file_name != "":
                    news.to_csv(file_name, encoding="utf-8", mode="a", index=False)
            except Exception as e:
                print(e)
                pass

        # +news['fourgram']+news['fivegram']
        news["ngram"] = news["bigram"] + news["trigram"]
